	  ...
	  render: function () {
	    return (
	      React.DOM.div({className: "stage"}, 
	        React.DOM.div({style: this.getStyles(1), className: this.props.config.cssClass + ' auth0-banner'}, 
	        ...
	        React.DOM.div({className: "clone-wrapper"}, 
	          React.DOM.div({ref: "clone", style: this.getStyles(2), className: this.props.config.cssClass + ' auth0-banner clone'}, 
	            React.DOM.div({
	              spellCheck: "false", 
	              className: "copy", 
	...
	var Index = React.createClass({displayName: 'Index',
	  ...
	  render: function () {
	    ...
	    var templateItem = function(item) {
	      return React.DOM.option({value: item.id}, item.name, " (", item.width, "x", item.height, ")");
	    };

	    return (
	      React.DOM.div({className: "index"}, 
	        React.DOM.p(null, 
	          React.DOM.h2(null, "Choose a Banner"), 
	          React.DOM.select({onChange: this.handleTemplateChange, value: c.state.bannerTemplate.id}, 
	            BannersCollection.map(templateItem)
	          )
	        ), 
	        Banner({ref: "editedBanner", config: template, updateDownload: this.generateURI}), 
	        React.DOM.a({href: this.state.banner, ref: "downloadButton", download: 'banner-' + size.w + 'x' + size.h + '.png', className: "btn btn-primary btn-lg"}, "Get my banner ", React.DOM.span({className: "icon icon-budicon-433"}))
	      )
	    );
	  }
	});
